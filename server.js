...
app.post("/api/login", async (req,res) => {
  try {
    ...
    if (!user && email === "demo@panelmarket.com") {
      const passwordData = hashPassword("12345678");
      const { data:demo, error } = await supabase.from("users").insert({ name:"Demo Kullanıcı", email:"demo@panelmarket.com", password_hash:passwordData.hash, password_salt:passwordData.salt, balance:5000, is_admin:true }).select("*").single();
      if (error) throw error;
      user = demo;
      console.log("DEMO ADMIN HESABI OLUŞTURULDU.");
    }
    if (user && email === "demo@panelmarket.com" && user.is_admin !== true) {
      const { data:updatedDemo, error:updateError } = await supabase.from("users").update({ is_admin:true }).eq("id",user.id).select("*").single();
      if (updateError) throw updateError;
      user = updatedDemo;
      console.log("DEMO HESABI ADMIN OLARAK GÜNCELLENDİ.");
    }
    ...
    res.status(500).json({ ok:false, success:false, authenticated:false, error:"Giriş sırasında bir hata oluştu." });
  }
});
